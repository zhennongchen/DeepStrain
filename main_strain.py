...
ES_use = 'ES_by_AI_volume' # 'ES_by_visual' or 'ES_by_AI_volume'

# main script for ED and ES:
for patient_index in range(0, patient_list.shape[0]):
    ...
    ES = patient_row[ES_use].values[0].astype(int)
    print(patient_id, ED,ES)

    save_folder = os.path.join(main_path, 'results/strain/', patient_id, 'ED_' + ES_use)
    ...
    if ES_use == 'ES_by_visual':
        mvf_file = os.path.join(main_path, 'results/trained/mvf', patient_id, 'mvf_ED_ES.nii.gz')
    else:
        mvf_file = os.path.join(main_path, 'results/trained/mvf', patient_id, 'mvf_ED'+str(ED) + '_tf' + str(ES) + '.nii.gz')

    print('mvf file: ', mvf_file)
    ...
    if os.path.isfile(os.path.join(os.path.dirname(save_folder), 'insertion_points.npy')):
        insertion_points = np.load(os.path.join(os.path.dirname(save_folder), 'insertion_points.npy'))
...
# main script for all time frames (need to run ED ES first):

# for patient_index in range(25,26):#patient_list.shape[0]):
#     patient_id_num = patient_list['OurID'].iloc[patient_index]
#     patient_id = ff.XX_to_ID_00XX(patient_id_num)
#     patient_row = spreadsheet.loc[spreadsheet['OurID'] == patient_id_num]
#     ED = patient_row['ED'].values[0].astype(int)
...
#         if os.path.isfile(os.path.join(save_folder, 'wtci.npy')) == True:
#             print('already did this frame, skip')
#             continue
